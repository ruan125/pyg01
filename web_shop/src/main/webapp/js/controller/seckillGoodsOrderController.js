 //控制层 
app.controller('seckillGoodsOrderController' ,function($scope,$controller,$location,typeTemplateService ,itemCatService,uploadService,seckillGoodsOrderService){
	
	$controller('baseController',{$scope:$scope});//继承
	
    //读取列表数据绑定到表单中  
	$scope.findAll=function(){
        seckillGoodsOrderService.findAll().success(
			function(response){
				$scope.list=response;
			}			
		);
	...
	$scope.findPage=function(page,rows){
        seckillGoodsOrderService.findPage(page,rows).success(
			function(response){
	...
	$scope.findOne=function(){	
		...
		// alert(id);
        seckillGoodsOrderService.findOne(id).success(
			function(response){
	...
	$scope.save=function(){	
		...
		if($scope.entity.id!=null){//如果有ID
			serviceObject=seckillGoodsOrderService.update( $scope.entity ); //修改
		}else{
			serviceObject=seckillGoodsOrderService.add( $scope.entity  );//增加
		}				
	...
	$scope.dele=function(){			
		//获取选中的复选框			
        seckillGoodsOrderService.dele( $scope.selectIds ).success(
			function(response){
				if(response.success){
    ...
    $scope.search=function(page,rows){
        seckillGoodsOrderService.search(page,rows,$scope.searchEntity).success(
            function(response){
                $scope.list=response.rows;
                $scope.paginationConf.totalItems=response.total;//更新总记录数
            }
